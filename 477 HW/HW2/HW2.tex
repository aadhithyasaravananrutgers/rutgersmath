...
\newline
\begin{parts}
    \part A total of 4 steps have to be taken to the right and 3 steps have to be taken up. The total number of ways to get from A to B is the number of ways to arrange the sequence of steps. This is equal to choosing 4 items from 7, which is \(\binom{7}{4}\) ways.
    \newline
\end{parts}
...
\newline
\begin{parts}
    \part Each set of twins can be treated as one item, so we first look for the number of ways to order 3 items, which is \(3!\) ways. However, since each pair of twins can be arranged in 2 ways, we have to multiply by \(2^3\) to get a total of \(3! \cdot 2^3\) ways.
    \newline
\end{parts}
